...
func TestRunConfigInitCommandStdout(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "til-consensus coding 裁决配置") {
...
func TestRunConfigInitCommandUsesDefaultPath(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "config template written:") || !strings.Contains(buf.String(), filepath.Base(path)) || !strings.Contains(buf.String(), "(quickstart)") {
