...
- `profile preflight` 会执行带 schema 的 `{"ok": true}` 最小探测；API provider 默认使用 `max_output_tokens=2048`，CLI provider 不支持在配置里声明 output-token budget。相对 `output.directory` 按当前执行目录解析；如果需要临时改目录，可加 `--output ./out/{requestId}`
